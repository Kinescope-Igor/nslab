...
function micConstraintsFor(mode) {
...
async function reinitContext(mode) {
...
async function applyMode(mode) {
    if (!state.context || !state.source || !state.analyser || !state.recorderDest || !state.capture)
        return;
    if (state.node) {
