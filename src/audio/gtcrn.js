...
export async function processFrameAsync(frame) {
    if (!loadedPromise)
        throw new Error('gtcrn not initialised');
    ...
    st.convCache = outputs.conv_cache_out;
    st.traCache = outputs.tra_cache_out;
    st.interCache = outputs.inter_cache_out;
    // Развернуть Hermitian 257 → 512 complex.
    const enhData = outputs.enh.data;
